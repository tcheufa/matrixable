...
fn test_new () {
        ...
        // let m1 = MatrixView::<u8>::new_with_init(5, 2, 2);
...
fn test_ops() {
        ...
        // Mul with integer as rhs
//                let m = matrix![2; 2, 2] * Multiply(2);
//                assert_eq!(matrix![[4, 4], [4, 4]], m);

        // Mul with integer as lhs
//                let m =  Multiply(2) * matrix![2; 2, 2];
//                assert_eq!(matrix![[4, 4], [4, 4]], m);
...
//                 // unit
//                 let mut m = matrix![[0]];
//                 assert_eq!(&mut matrix![[0]], m.transposed());
// 
//                 // one-dimensional
//                 let mut m = matrix![[0], [1], [2], [3]];
//                 assert_eq!(&mut matrix![[0, 1, 2, 3]], m.transposed());
//                 assert_eq!(&mut matrix![[0], [1], [2], [3]], m.transposed());
// 
//                 // square
//                 let mut m = matrix![[0, 1, 2], [3, 4, 5], [6, 7, 8]];
//                 assert_eq!(&mut matrix![[0, 3, 6], [1, 4, 7], [2, 5, 8]], m.transposed());
// 
//                 // two-dimensional non-square
//                 let mut m = matrix![[0, 1], [2, 3], [4, 5]];
//                 assert_eq!(&mut matrix![[0, 2, 4], [1, 3, 5]], m.transposed());
// 
//         }
