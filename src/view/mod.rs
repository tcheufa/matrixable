...
use crate::panic_if_bad_size;
use crate::traits::{ Matrix, MatrixMut, SwapDimensions, MatrixFromIter };
...
impl<'a, T: Clone> MatrixFromIter<&'a T> for MatrixView<T> {
    fn from_iter<I>(into_iter: I, rows: usize, cols: usize) -> Self 
    where I: IntoIterator<Item = &'a T> 
    {
        let mut m = Self::with_capacity(rows, cols);
        m.iter_mut().zip(into_iter.into_iter())
                    .for_each(|(x, &ref x2)| *x = x2.clone());
                    
        m
    }
}
impl<T> MatrixFromIter<T> for MatrixView<T> {
    fn from_iter<I>(into_iter: I, rows: usize, cols: usize) -> Self 
